...
function addListeners (boardNode) {
  for (var i = 0; i < boardNode.children.length; i++) {
    ...
    boardNode.children[i].addEventListener('contextmenu', contextMenu, false)
    boardNode.children[i].addEventListener('touchstart', touchStart, false)
    boardNode.children[i].addEventListener("touchend", touchEnd, false)
  }
}
...
function touchStart (ev) {
  var evt = ev
  touchtimer = setTimeout(onlongtouch.bind(this, evt), touchduration)
}
...
function showCell (evt) {
  ...
  if (evt.target.classList.contains('mine')) {
    displayMessage("you lose!<br><br>select level:")
    revealMines()
    removeListeners()
    return
  }
...
function markCell (evt) {
  evt.classList.toggle('marked')
  var idx = getCellIndex(getRow(evt), getCol(evt))
  var cell = board.cells[idx]
...
function revealMines () {
  stopTimer()
  makeArray(document.querySelectorAll('.mine, .hidden'))
