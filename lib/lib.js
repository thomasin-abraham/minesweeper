...
function addListeners (boardNode) {
  for (var i = 0; i < boardNode.children.length; i++) {
    ...
    boardNode.children[i].addEventListener('contextmenu', contextMenu)
    boardNode.children[i].addEventListener('touchstart', touchStart)
  }
}
...
function touchStart (ev) {
  touchtimer = setTimeout(onlongtouch.bind(this), touchduration)
}
